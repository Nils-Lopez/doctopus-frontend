...
import { faRotateLeft, faAngleRight, faAngleLeft, faArrowDownAZ, faEyeSlash, faMagnifyingGlass } from '@fortawesome/free-solid-svg-icons'

const Show = ({client, docs, tag, setDisplayDoc, handleSearchTag, handleBack, setAlert, applicationSettings}) => {
  ...
  const [updateTag, setUpdateTag] = useState(null)
    ...
    useEffect(() => {
      ...
      if (filters && filters.value !== "all") {
          childs.map((child) => {
              if (child && child.types && child.types[0] && child.types[0]._id && filters && filters.value) {
      ...
      if (searchInput.length > 2) {
          ...
          newFilteredList.map((child) => {
              if (child && child.title && child.title !== "" && child.title.toLowerCase().replace(/[^\w ]/, "").includes(searchInput.toLowerCase().replace(/[^\w ]/, ""))) {
    ...
    <div className="is-flex is-justify-content-space-between">
        ...
        <h3 className="subtitle is-2 has-text-grey mt-0 pt-0 mb-6"><strong className="has-text-primary">{getContent(tag.title, i18n.language)}</strong></h3>
          ...
          <div className="is-flex is-justify-content-start">
            {!childsDataLoading ? <span className="tag is-primary is-medium is-rounded">{childsData.length} {t('results')}</span> : null }
