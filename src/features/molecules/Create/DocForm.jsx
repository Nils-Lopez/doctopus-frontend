...
const DocForm = ({
  ...
  useEffect(() => {
    if (responseCreateDoc && responseCreateDoc.success) {
      setAlert({
        type: "success",
        message: {
          en: "Document has been successfully created.",
          fr: "Le document a été créé avec succès",
        },
      });
      if (draft && setClient) {
        setClient({
          ...
          user: {
            ...client.user,
            drafts: [...client.user.drafts, responseCreateDoc.data],
          },
        });
      }
      setLoading(false);
      ...
      setAlert({
        type: "error",
        message: {
          en: "An error occured while creating document.",
          fr: "Une erreure est survenue lors de la création du document",
        },
      });
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (responseUpdateDoc && responseUpdateDoc.success) {
      setAlert({
        type: "success",
        message: {
          en: "Document has been successfully updated.",
          fr: "Le document a été mis à jour avec succès",
        },
      });
      setLoading(false);
      ...
      setAlert({
        type: "error",
        message: {
          en: "An error occured while updating document.",
          fr: "Une erreure est survenue lors de la mise à jour du document",
        },
      });
      setLoading(false);
      setDataUpdate(false);
  ...
  useEffect(() => {
    if (!dataUpdate) {
      if (
        template &&
        (template.tag_defaults !== selectedTags ||
          template.type_defaults !== selectedTypes ||
          (template.languages && template.languages.defaults !== selectedLangs))
      ) {
        ...
        selectLang(
          template.langs
            ? template.langs.map((l) => {
                return {
                  value: l._id,
                  label: getContent(l.title, i18n.language),
                };
              })
            : []
        );
  ...
  const { responseFindAllDocTemplates, findAllDocTemplates } =
    useDocTemplates();
  ...
  useEffect(() => {
    docTemplates.map((template) => {
      if (
        ...
        selectTemplate({
          value: template.schema_name,
          label: template.schema_name,
        });
        setTemplateModel(template);
      }
    });
  ...
  useEffect(() => {
    if (!subTemplate || !templateModel.schema_childs.includes(subTemplate)) {
      ...
      if (model.tag) {
        model.tag_defaults = [
          ...new Set([...model.tag_defaults, ...subTemplate.tag_defaults]),
        ];
      }
      if (model.type) {
        model.type_defaults = [
          ...new Set([...model.type_defaults, ...subTemplate.type_defaults]),
        ];
      }
      if (model.support_role) {
        model.support_role_defaults = [
          ...new Set([
            ...model.support_role_defaults,
            ...subTemplate.support_role_defaults,
          ]),
        ];
      }
      ...
      if (model.parent_role) {
        model.parent_role_defaults = [
          ...new Set([
            ...model.parent_role_defaults,
            ...subTemplate.parent_role_defaults,
          ]),
        ];
      }
      if (model.parent_entity) {
        model.parent_entity_defaults = [
          ...new Set([
            ...model.parent_entity_defaults,
            ...subTemplate.parent_entity_defaults,
          ]),
        ];
      }
      if (model.parent_person) {
        model.parent_person_defaults = [
          ...new Set([
            ...model.parent_person_defaults,
            ...subTemplate.parent_person_defaults,
          ]),
        ];
      }
      if (model.parent_project) {
        model.parent_project_defaults = [
          ...new Set([
            ...model.parent_project_defaults,
            ...subTemplate.parent_project_defaults,
          ]),
        ];
      }
  ...
  useEffect(() => {
    if (dataUpdate) {
      ...
      setPubliDateValue(
        dataUpdate.publishedAt ? dataUpdate.publishedAt.split("T")[0] : ""
      );
  ...
  useEffect(() => {
    if (responseFindBookByIsbn && responseFindBookByIsbn.success) {
      setAlert({
        type: "success",
        message: {
          en: "A book was found in our data.",
          fr: "Un livre a été trouvé dans nos données",
        },
      });
      ...
      setAlert({
        type: "error",
        message: {
          en: "No book was found in our data.",
          fr: "Ce livre n'a pas été trouvé dans nos données",
        },
      });
    }
    ...
    if (autoCompletion) {
      ...
      if (autoCompletion.date) {
        if (autoCompletion.date.includes("-")) {
          ...
          setDateValue(
            autoCompletion.date.split("-")[2] +
              " " +
              new Date(autoCompletion.date).toString().split(" ")[1] +
              " " +
              autoCompletion.date.split("-")[0]
          );
        } else {
          setDateValue(autoCompletion.date);
        }
      }
      if (autoCompletion.publishedAt)
        setPubliDateValue(autoCompletion.publishedAt);
  ...
  return loading || isbnLoading ? (
    ...
    <div>
      <div className="columns">
            ...
            <div className="field">
              ...
              <SelectForm
                ...
                options={[
                  ...
                  { value: "organisation", label: t("organization") },
        ...
        {template &&
        template.schema_childs &&
        template.schema_childs[0] &&
        !dataUpdate ? (
          <>
            <div className="column is-one-third">
              <div className="field">
                ...
                <SelectForm
                  applicationSettings={applicationSettings}
                  select={handleSelectSubTemplate}
                  selected={selectedSubTemplate}
                  options={templateModel.schema_childs.map((schema) => {
                    return {
                      value: schema.schema_name,
                      label: schema.schema_name,
                    };
      ...
      {showIdentityForm ? (
        <>
          {scanner ? (
            <>
              <span
                ...
                className="indextag  has-text-info ml-3 pt-2 pb-0 subtitle is-4">
            ...
            <>
              {template && template.support_eanIsbn ? (
                <div className="field">
                  <label className="label has-text-left">{t("ean-isbn")}</label>
          ...
          {template && template.languages && template.languages.exist ? (
            <div className="columns">
              ...
              <div className="column">
                <div className="field" id="docLang">
                  <label className="label has-text-left">{t("language")}</label>
                  <SelectForm
                    applicationSettings={applicationSettings}
                    select={selectLang}
          ...
          {template && template.description ? (
            <div className="field mt--1" id="docDesc">
              <label className="label has-text-left">{t("description")}</label>
              <textarea
                className="textarea"
                value={i18n.language === "fr" ? descFrValue : descEnValue}
          ...
          {template && template.tag ? (
            <SearchTagsForm selectedTags={selectedTags} selectTag={selectTag} />
          ) : null}
          {template && template.copyrights ? (
            <div className="field mt-2">
              <label className="label has-text-left">{t("copyrights")}</label>
              <input
                type="text"
                className="input"
                ...
                <div className="column">
                  <div className="field">
                    ...
                    <input
                      ...
                      onChange={handlePubliDateChange}
                    />
                  </div>
                </div>
                <div className="column">
          ...
          {template && template.support_pages ? (
            <div className="field">
              <label className="label has-text-left">{t("pages")}</label>
          ...
          {template && template.support_duration ? (
            <div className="field">
              <label className="label has-text-left">{t("duration")}</label>
          ...
          {template && template.support_thumb ? (
            <div className="field">
              <label className="label has-text-left">{t("thumbnail")}</label>
      ...
      {showSupportForm ? (
        <SupportForm
          ...
          setPendingSupports={setPendingSupports}
          selectedRoles={selectedRoles}
          selectRole={selectRole}
          pendingExemplaries={pendingExemplaries}
          setPendingExemplaries={setPendingExemplaries}
          template={template}
      ...
      {showParentForm ? (
        <ParentForm
          ...
          selectedProj={selectedProjects}
          selectProj={selectProject}
          template={template}
          client={client}
          setAlert={setAlert}
