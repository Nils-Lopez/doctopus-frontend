...
const SupportForm = ({
  ...
  const deleteSupportPreview = (support) => {
    const filtered = pendingSupports.filter((supp) => {
      return supp !== support;
    });
  ...
  return (
    <>
      ...
      {template && template.support_pdf ? (
        <div className="field">
          <label className="label has-text-left">PDF</label>
