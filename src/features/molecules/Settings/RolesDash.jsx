          ...
          <div className="field max-50">
            <div className="control has-icons-left">
              <input
                type="text"
                className="input"
                placeholder={merging ? t("merge-with") : t("title")}
                value={query}
                onChange={(e) => {
                  e.preventDefault();
          ...
          <>
            {merging ? (
              <RoleBlock
                ...
                merging={merging}
                setMatchingRoles={setMatchingRoles}
  ...
  return (
    <>
      <div className="panel-block columns">
        <p className="column is-4 has-text-left">
          {!update ? (
            getContent(role.title, "fr")
          ) : (
            <>
      ...
      {matchingRoles[0] && merging
        ? matchingRoles.map((matchingRole) => {
            if (matchingRole._id !== merging._id) {
              return (
                <Fragment key={matchingRole._id}>
                  <div className="panel-block columns">
                    <p className="column is-4 has-text-left">
                      {getContent(matchingRole.title, "fr")}
                    </p>
                    <p className="column is-4 has-text-left">
                      {getContent(matchingRole.title, "en")}
                    </p>
                    <div className="column is-4 is-flex is-justify-content-end">
                      <button
                        className="button is-primary mr-2"
