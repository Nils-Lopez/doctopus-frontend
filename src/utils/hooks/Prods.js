...
const useProds = () => {
  ...
  return {
    ...
    findProdById: async function (id) {
      const production = await apiFetch(
        "https://api.aml-cfwb.be/scapin/spectacle.svc/anon/get/" + id
      );

      const team = await apiFetch(
        "https://api.aml-cfwb.be/scapin/spectacle.svc/anon/getequipe/" + id
      );

      const productor = await apiFetch(
        "https://api.aml-cfwb.be/scapin/spectacle.svc/anon/getproduction/" + id
      );
      ...
      await Promise.all(
        ids.map(async (id) => {
          ...
          if (production && production[0]) {
            ...
            const parentDocs = await apiFetch(
